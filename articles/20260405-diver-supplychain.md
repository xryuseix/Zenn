...
こんにちは、DIVER OSINT CTFの運営メンバーで主にインフラ周りを担当しています、xryuseixです。特に今年に入ってからソフトウェアサプライチェーン攻撃の脅威が顕著に現れ、対応が求められていますが、これはDIVER OSINT CTFの作問環境に関しても例外ではありませんでした。

そこで、この記事では、

1. **なぜDIVER OSINT CTFがソフトウェアサプライチェーン攻撃対策をやるのか**
2. **実際にどんな対策を実施したのか**

をお話ししていこうと思います。

## なぜDIVER OSINT CTFがソフトウェアサプライチェーン攻撃対策をやるのか
...
そこで、DIVER OSINT CTFの運営基盤全体を見直し、ソフトウェアサプライチェーン攻撃を「防ぐ」ための対策と、万が一侵害が起きた場合に「追跡する」ための仕組みを整備しました。
...
ところで、[suzuki-shunsuke/pinact](https://github.com/suzuki-shunsuke/pinact)自体がソフトウェアサプライチェーン攻撃くらったら終わりでは？と思ったので、こんな`docker-compose.yml`を作って、`pinact`コマンドで動くようにしました。
...
- 入力値のバリデーション
  - action.ymlの`inputs`で受け取る値に対して、許可リストによるチェックを行っています(とはいえこれはソフトウェアサプライチェーン関係なく、ただのバグを減らすためのバリデーションな気がします)
...
- 全ての内製ツールをGoで書き直した
  ...
  - とはいえ、WebアプリケーションのフロントエンドはJS/TSで実装されているため、npm依存を完全にゼロにはできていません。この部分はTakumi Guardやdevcontainerによるリスクの軽減をしています
...
[Takumi Runner](https://flatt.tech/takumi/features/runner)とは、[GMO Flatt Security株式会社](https://flatt.tech/)(弊社)が提供する、GitHub Actionsのソフトウェアサプライチェーン攻撃対策のためのSelf Hosted Runnerです。[eBPF](https://ebpf.io/)等の技術を用いてCI/CDパイプライン上のプロセス・ネットワーク通信・ファイルアクセスなどを可視化してくれるため、ソフトウェアサプライチェーン攻撃発生時の調査に役立ちます。
...
この記事では、DIVER OSINT CTFの運営基盤に対して行ったソフトウェアサプライチェーン攻撃対策を紹介しました。SHA Pinning・最小権限化・インジェクション対策といった「防御」と、Takumi Runnerによる「追跡」の両面からアプローチを行いました。
